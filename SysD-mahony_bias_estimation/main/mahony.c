                         ...
                         float ax, float ay, float az)
{
    ...
    // --- bias compensation & estimation ---
    {
        float raw_gx = gx, raw_gy = gy, raw_gz = gz;
        // remove current bias estimate
        gx = raw_gx - gyroBiasX;
        gy = raw_gy - gyroBiasY;
        gz = raw_gz - gyroBiasZ;
        // update bias via LPF on raw reading
...
void mahony_set_quaternion(float w, float x, float y, float z)
{
 ...
 /**
  * @brief   Retrieve current gyro bias estimates (rad/s).
  *          (These are the values being subtracted each update.)
  */
