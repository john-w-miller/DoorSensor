...
void app_main(void)
{
    ...
    // setup minute‐reporting
    TickType_t lastReportTime = xTaskGetTickCount();
    float lastDriftZ = 0.0f;    
    ...
    while (true) {
        ...
        // every 60 s, log estimated Z‐bias (drift) and its delta
        {
            TickType_t now = xTaskGetTickCount();
            if ((now - lastReportTime) >= pdMS_TO_TICKS(60000)) {
                float bx, by, bz;
                mahony_get_gyro_bias(&bx, &by, &bz);
                float driftZ = bz * RAD_TO_DEG;
                float delta  = driftZ - lastDriftZ;
                ESP_LOGI(TAG,
                         "Gyro drift estimate Z = %+6.3f °/s, Δ = %+6.3f °/s , Yaw = %+6.2f°",
                         driftZ, delta, yaw_filt*RAD_TO_DEG);
                lastDriftZ = driftZ;
                lastReportTime = now;
            }
        }
